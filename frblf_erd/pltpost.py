        ...
        if bolupp:
            if len(levels)>0:
                for ls in [0.68, 0.95]:
                    ...
                    print('%d-th parameter sigma= %f lv= %f rv= %f thre= %f' % (vari, ls, lv, rv, thre))
...
if __name__ == '__main__':
    ...
    bolupp = args.bolupp
    bolout = args.bolout
    # 修复了 n_params 与 nest_samp.py 实际参数个数 (6个) 的匹配问题，避免维度报错
    ...
    rdat = np.array([[0, 4], [-3.0, 1.0], [38, 44], [36, 42], [-0.4, 0.4], [0, 0.8]])
    # parameter ranges for mock data
    # rdat = np.array([[2, 5], [-2.0, -0.5], [39, 44], [37, 42], [0, 0.8], [0, 0.8]])
    ...
    if bolout:
    	...
    	plot2dposterier_withconf(mxchain, vlik, par=vpar, indx=[], rangedat=rdat, rate=1.0, levels=[0.68, 0.95], 
            labels=[r'$\log\,\phi^*$', r'$\alpha$', r'$\log E_{\rm iso}^*$', r'$\log E_{\rm iso,0}$', r'$\mu_w$', r'$\sigma_w$'])
    	plt.suptitle(tit)
    	plt.savefig(fout)
