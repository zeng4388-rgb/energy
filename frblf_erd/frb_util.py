...
class Cosmology:
    ...
    def DMeq(self, z, dme, dmhost):
        ...
        dmi = self.DispersionMeasure_IGM(z_safe)
        dmh = dmhost/(1+z_safe)
        return dmi + dmh - dme
...
class AstroDistribution:
    ...
    def log_distr_fdmwz(self, dnu, logflux, dme, logw, z, alpha, logls, logl0, mu, sigma, gtype=None):
        ...
        fluence = flux * np.power(10., logw)
        loge = np.log10(self.cos.Energy(z, flu=fluence, dnu=dnu))
        logint1 = self.log_IntBeam(loge, alpha, logls, logl0)
        #print logint1
    ...
    def Norm1D(self, sn0, bw, npol, g, tsys, dnu, alpha, logls, logl0, mu, sigma):
        ...
        for z in vz:
            ...
            flu_t = ft * vw
            et = self.cos.Energy(z, flu_t, dnu)
            loget = np.log10(et)
            ind = loget < logl0
            loget[ind] = logl0
            int_eps = np.zeros(loget.shape)
            for i in np.arange(len(loget)):
                int_eps[i] = np.sum(self.IntLum(veps, alpha, logls, loget[i]*np.ones(veps.shape))/veps/np.log(2)*stepeps)
    ...
    def rate_2d(self, sn0, bw, npol, g, tsys, dnu, phis, alpha, logls, logl0, mu, sigma):
        ...
        for z in vz:
            ...
            for i in range(len(loget)):
                int_eps[i] = np.sum(phis*self.ad.IntLum(veps, alpha, logls, loget[i]*np.ones(veps.shape))/veps/np.log(2)*stepeps)
