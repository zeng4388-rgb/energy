...
if __name__ == '__main__':
    ...
    if bolupper:
        vpara=np.array([-3.0, -3.0, 38.0, 1e34, -1, 0.1])
        vparb=np.array([4.47, 1.1, 45.0, 1e42, 2, 1.0])
    else:
        vpara=np.array([1e-3, -3.0, 38.0, 34.0, -1, 0.1])
        vparb=np.array([3e4, 1.1, 45.0, 42.0, 2, 1.0])
    ...
    # create nest_out/samp/ directory if not exist to prevent path errors
    import os
    out_dir = os.path.join(os.path.dirname(__file__), 'nest_out', 'samp')
    os.makedirs(out_dir, exist_ok=True)
    basename = os.path.join(out_dir, fout)
